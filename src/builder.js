...
const pageHtml = fs.readFileSync(`./views/${pageToBuild}/${pageToBuild}.page`, 'utf-8');
...
function includeComponent(html, componentName, componentHtml) {
    ...
    if (cssName == 'routing')
        cssName = pageToBuild;
    componentHtml = `<link rel="stylesheet" href="${publicPath}/stylings/${cssName}.css">` + componentHtml;

    componentHtml = addCssPrefixes(componentHtml, cssName);
    return html.replaceAll(`<${componentName}-component><\/${componentName}-component>`, componentHtml);
}
...
function parseHtml(html, routedPage) {
    const formIds = [];
    return html.replace(/<(\w+)-component><\/\w+-component>/g, (replacer, componentName) => {
